...
Backend de la aplicación "Shipwreckeds". Provee **autenticación** (AWS Cognito), **gestión de partidas**, **caché distribuida en Redis** y el **motor de juego** con soporte para comunicación en tiempo real vía **WebSocket/STOMP**.
...
| :------------------ | :---------------- | :---------------------------------------------------------------------------------------------- |
| **Lenguaje**        | Java              | 17 (ver `pom.xml`)                                                                              |
| **Framework**       | Spring Boot 3.3.5 | Starters Web, WebSocket, Data Redis, Integration                                                |
| **Tiempo real**     | WebSocket + STOMP | Broker simple embebido (`/topic`) con prefijo `/app`                                            |
| **Capa de estado**  | Redis/Valkey      | Plantillas dedicadas (`matchRedisTemplate`, `playerSessionRedisTemplate`) y `RedisLockRegistry` |
| **Autenticación**   | AWS Cognito       | Cliente HTTP propio (`HttpCognitoAuthenticationClient`)                                         |
| **Build y calidad** | Maven 3+, JaCoCo  | Empaquetado, ejecución y mediciones de cobertura                                                |
...
2.  **WebSocket/STOMP:** El Cliente establece una conexión **WebSocket/STOMP** con el `GameEngine` para el intercambio de eventos en tiempo real (movimientos, chat, votaciones).
    - **Clientes** se suscriben a tópicos (`/topic/game/{matchId}`) para recibir el estado del juego (broadcast) que el `GameEngine` publica.
...
| **Protocolo de mensajería: STOMP sobre WebSocket** | Añade una capa estructurada (tópicos, colas lógicas, suscripciones) que facilita el diseño de eventos _pub/sub_ del juego.                                        | Enrutamiento sencillo por destino, control de suscripciones por sesión, simplifica la integración con Spring.                        |
| **Broker WebSocket embebido**                      | Actualmente se utiliza el broker simple de Spring (`enableSimpleBroker`).                                                                                         | Configuración ligera orientada a desarrollo; las réplicas ya comparten eventos en Redis pub/sub mediante `DistributedWsBroadcaster`. |
...
- **Escalabilidad:** Redis pub/sub ya comparte eventos entre réplicas y sostiene el escalado horizontal del backend.
...
│   │   ├── ShipwreckedsBackendApplication.java
│   │   ├── config/
...
│   │   ├── integration/cognito/
│   │   │   ├── CognitoAuthenticationClient.java
│   │   │   ├── CognitoTokens.java
│   │   │   └── HttpCognitoAuthenticationClient.java
│   │   ├── model/
│   │   │   └── dominio y DTOs (Match, Player, GameState, etc.)
│   │   └── service/
│   │       ├── AuthService.java
│   │       ├── GameEngine.java
│   │       ├── MatchService.java
...
└── test/java/com/arsw/shipwreckeds/ (tests unitarios e integración)
...
- **Java 17** (versión objetivo del proyecto)
- **Maven 3.9+**

### Compilación y ejecución

Compilar el proyecto (puedes omitir las pruebas si necesitas un build rápido):

```bash
# Empaquetado completo
mvn clean package

# Empaquetado rápido (omite pruebas)
...
# Opción 2: usar el plugin de Spring Boot
mvn spring-boot:run
```
